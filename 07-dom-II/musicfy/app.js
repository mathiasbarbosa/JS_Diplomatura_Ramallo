...
const close_player = document.querySelector('#close_player')
...
function createCard(artista){
    ...
    img.setAttribute('alt', artista.nombre)

    div.append(h2, h3, small, p, p_ultimoLanzamiento, img)
    containerArtistas.appendChild(div)
}

function renderedCards(artistas){
...
function openNavPlayer(){
    document.addEventListener('click', (e) => {
        const nav_lower_player = document.querySelector('#nav_lower_player')
        console.dir(e.target);
        console.dir(e.target.parentNode);
        console.dir(e.target.parentNode.classList[0]);
        
        if (e.target.parentNode.classList[0] === 'card' ) {
...
function filterbycategory(category) {
    const filter =  artistas.filter(artista => artista.genero === category)
...
document.addEventListener('DOMContentLoaded', () =>{
    ...
    close_player.addEventListener('click', () => {
        nav_lower_player.classList.add('hiden')
    })

    openNavPlayer()
    let categorys = document.querySelectorAll('.item_genero')
