...
const nav_profile_saludo = $.querySelector('#nav_profile > h3')
const nav_profile = $.querySelector('#nav_profile')
const theme_mode = $.querySelector('#theme_mode')
...
const filterByCategory = () => {
    ...
    items_genero.forEach((item) => {
        item.addEventListener('click', (e) => {
            ...
            categoria === 'Todos' ? renderedCards(artistas) : renderedCards(filterCategory(categoria))
...
const searchArtist = () => {
    const search = $.querySelector('#formSearch > input')
    search.addEventListener('input', (e) => {
        let find = e.target.value
        let result = artistas.filter(artista => artista.nombre.toLocaleLowerCase().includes(find.toLocaleLowerCase()))
        renderedCards(result)
    })
}
...
const togleTheme = () => {
    theme_mode.addEventListener('click', () => {
        ...
        nav_profile.classList.toggle('bg-dark-nav')
        $.querySelectorAll('.card').forEach((card) => {
            card.classList.toggle('bg-dark-card')
        })
        $.querySelectorAll('.item_nav').forEach((card) => {
            card.classList.toggle('bg-dark-item_nav')
        })
        $.querySelectorAll('.item_genero').forEach((card) => {
            card.classList.toggle('bg-dark-item-genero')
        })
    })
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    console.log(usuario);
    usuario ? nav_profile_saludo.textContent = `Hola ${usuario.firstName}` : nav_profile_saludo.textContent = 'Hola'
